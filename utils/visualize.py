...
PLOT_NAMES = dict(generate_samples="samples.png",
                  ...
                  shuffle_reconstructions="shuffle_reconstructions.png"
                  )
...
class Visualizer():
    ...
    def _get_traversal_range(self, mean=0, std=1):
        ...
        print('max_traversal', max_traversal)
    ...
    def _save_or_return_2(self, to_plot, size, filename, is_force_return=False):
        """Create plot and save or return it."""
        to_plot = F.interpolate(to_plot, scale_factor=self.upsample_factor)

        to_plot = to_plot[torch.randperm(to_plot.size(0))[12:4+12]]

        b, n, c, h, w = to_plot.shape

        to_plot = to_plot.permute(1, 0, 2, 3, 4)

        to_plot = to_plot.reshape(n//2, 2, b, c, h, w).permute(0, 2, 1, 3, 4, 5)

        to_plot = to_plot.reshape(n//2, 2*b, c, h, w)

        to_plot = to_plot.reshape(b*n, c, h, w)
    ...
    def intervene_reconstruct(self,
                    data,
                    size=(8, 8),
                    is_original=True,
                    is_force_return=False):
        ...
        with torch.no_grad():
            originals = data.to(self.device)[:n_samples, ...]
            recs = self.model(originals)['shuffle_reconstructions']
        ...
        if recs.dim() == 4:
            ...
            return self._save_or_return(
                to_plot,
                size,
                PLOT_NAMES["shuffle_reconstructions"],
                is_force_return=is_force_return
                )
        
        elif recs.dim() == 5:
            to_plot = torch.stack([originals, torch.flip(originals, dims=(0,))],dim=1)
            to_plot = torch.cat([to_plot, recs.cpu()], dim=1)
            return self._save_or_return_2(
                to_plot,
                size,
                PLOT_NAMES["shuffle_reconstructions"],
                is_force_return=is_force_return
