...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        return {
            'stats_qzx': dist_statistics.view(-1, self.latent_dim, self.dist_nparams),
            'prior_feature': x
            }


if __name__ == '__main__':
    encoder = Encoder(img_size=(3, 64, 64), latent_dim=10)

    fake_img = torch.randn(1, 3, 64, 64)

    output = encoder(fake_img)

    stats_qzx = output['stats_qzx']
    print(stats_qzx.size())

    group_feature = output['group_feature']
    print(group_feature.size())