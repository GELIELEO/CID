...
class Encoder(nn.Module):
    ...
    def __init__(self, img_size, latent_dim=10, dist_nparams=2):
        ...
        self.latent_quantizer = LatentQuantize(
...
if __name__=='__main__':
    '''
    特征向量每一维度都有一个对应码本
    每个码本取值范围在-0.5~0.5，根据level的设置均匀取值
    码本的大小就是所有维度level各自大小的乘积，比如2x2x2

    '''
    latent_quantizer = LatentQuantize(
        levels=4,
        dim=3,
        codebook_dim=3,
        num_codebooks=2
    ...
    fake_latent = torch.randn(2, 3)
    out, indices, loss = latent_quantizer(fake_latent)
    # print(fake_latent, out, indices, loss)
    print(latent_quantizer.implicit_codebook.shape)
