...
class Model(nn.Module):
    ...
    def __init__(self, img_size, latent_dim, **kwargs):
        ...
        gestalt_root = '/home/ibasaw/causal_ws/WorldModel/disentangling-correlated-factors/tools/gestalt'
        image_list = os.listdir(gestalt_root)
        image_list.sort()
        ...
        for image in image_list:
            if 'black' in image:
                black = cv2.imread(os.path.join(gestalt_root, image), cv2.IMREAD_GRAYSCALE)
                self.black.append(torch.from_numpy(cv2.resize(black, (64, 64))).to('cuda')/255.0)
            if 'white' in image:
                white = cv2.imread(os.path.join(gestalt_root, image), cv2.IMREAD_GRAYSCALE)
                self.white.append(torch.from_numpy(cv2.resize(white, (64, 64))).to('cuda')/255.0)

        assert len(self.black) == len(self.white)
    ...
    def forward(self, x):
        ...
        idx = torch.randint(0, len(self.black), (1,))
        black = self.black[idx]
        white = self.white[idx]

        stats_qzx = self.encoder(x+black)['stats_qzx']
