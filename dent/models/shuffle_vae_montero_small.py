...
import dent.models.encoder.ibasaw_small
import dent.models.decoder.ibasaw_small
...
class Model(nn.Module):
    ...
    def __init__(self, img_size, latent_dim, **kwargs):
        ...
        self.dist_nparams = 2
        self.encoder = dent.models.encoder.ibasaw_small.Encoder(
            img_size, self.latent_dim, dist_nparams=self.dist_nparams)
        self.decoder = dent.models.decoder.ibasaw_small.Decoder(
            img_size, self.latent_dim)
        
        self.shuffle_dims = kwargs['shuffle_dims']
        self.momentum = kwargs['momentum']
        
        self.model_name = 'vae_ibasaw_small'


        self.mencoder = nn.Sequential(
            nn.Linear(256, self.latent_dim * self.dist_nparams)
        )
        self.mdecoder = nn.Sequential(
        ...
        for p in self.mencoder.parameters():
            p.requires_grad_(False)
        for p in self.mdecoder.parameters():
            p.requires_grad_(False)
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            for p1,p2 in zip(self.encoder.dist_statistics.parameters(), self.mencoder.parameters()):
                p2.data = me*p2.data + (1-me)*p1.data
            for p1,p2 in zip(self.decoder.lin1.parameters(), self.mdecoder.parameters()):
        ...
        shuffle_samples_qzx_rect = self.mdecoder(shuffle_sample_qzx)
        shuffle_stats_qzx_rect = self.mencoder(shuffle_samples_qzx_rect).view(-1, self.latent_dim, self.dist_nparams)
        # de-shuffle
        deshuffle_stats_qzx_rect  = self.deshuffle_latent(shuffle_stats_qzx_rect, latent_shuffled_idx, shuffle_history)
        ...
        if not self.training:
            ...
            for i in latent_to_swap:
                ...
                swap_results.append(
                    self.decoder(self.reparameterize(*swap_stats_qzx.unbind(-1))['samples_qzx'])['reconstructions']
                    )
                swap_results.append(
                    self.decoder(self.reparameterize(*swap_stats_qzx_.unbind(-1))['samples_qzx'])['reconstructions']
                    )
        ...
        return {
            ...
            'pair_1': (stats_qzx, deshuffle_stats_qzx_rect),
