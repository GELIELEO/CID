...
from .modules.dagness import dagness
from .modules.batch_dagness import batch_dagness
...
class Loss(dent.losses.baseloss.BaseLoss):
    ...
    def __call__(self, data, reconstructions, stats_qzx, is_train, **kwargs):   
        ...
        # mean_att = att[:,0]
        # mean_att = torch.where(mean_att.abs()>1e-1, torch.ones_like(mean_att)+mean_att-mean_att.detach(), torch.zeros_like(mean_att)+mean_att-mean_att.detach())
        # dn = batch_dagness(mean_att, att.shape[-1]).mean()
        # log_data['dagness'] = dn.item()
        # print(dn)
