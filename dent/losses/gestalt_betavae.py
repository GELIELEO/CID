...
class Loss(dent.losses.baseloss.BaseLoss):
    ...
    def __call__(self, data, reconstructions, stats_qzx, white, is_train, **kwargs):   
        self._pre_call(is_train)
        if isinstance(stats_qzx, torch.Tensor):
            stats_qzx = stats_qzx.unbind(-1)     

        log_data = {}
        rec_loss = _reconstruction_loss(data+white, reconstructions, distribution=self.rec_dist)
