...
Section('run', 'base run parameters').params(
    ...
    restore_to_new=Param(utils.Bool(), 
                         ...
                         default=False),
    restore_hyperparams=Param(utils.Bool(), 'Flag. If set, will also load hyperparameter from ckpt, i.e., lr and scheduler', default=True),
...
Section('rect', 'image reconstruction parameters').params(
    rect_coeff=Param(float, 'alpha value', default=1.0),
    )
...
Section('shuffled_betavae', 'higgins beta-VAE parameters').params(
    shuffled_coeff=Param(float, 'shuffled value', default=1.0),
    shuffled_target=Param(int, 'target to train', default=0),
    shuffle_dims=Param(int, 'number of dimensions to shuffle', default=1),
    momentum=Param(float, 'momentum', default=0.996),
    )
