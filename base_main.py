    ...
    if run_training:
        ...
        if info_dict['start_from_chkpt']:
            ...
            trainer.initialize_from_checkpoint(
                info_dict['chkpt_data'], uid=uid, restore_hyperparams=config['run.restore_hyperparams'])
        else:
            trainer.initialize()
                ...
                if "all" in config['viz.plots']:
                    config.collect({
                        'viz.plots': [
                            ...
                            "traversals", 'reconstruct-traverse', "gif-traversals", "shuffle_reconstructions"
                        ]
                    })
                ...
                for plot_type in config['viz.plots']:
                    ...
                    if plot_type == 'generate-samples':
                        ...
                        return_dict = viz.gif_traversals(
                            ...
                            n_latents=config['viz.nrows'],
                            loss_per_latent=loss_per_latent)
                    elif plot_type == "shuffle_reconstructions":
                        return_dict = viz.intervene_reconstruct(samples,
                                                                size=size
                                                                )
                    else:
